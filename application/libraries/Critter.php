...
class Template {
    ...
    public function load_view($view, $data = array(), $return = FALSE)
    {
        ...
        {
            $js[] = '<script src="' .$this->_ci->config->base_url(). assets_url('js/' . $js_file) . '"></script>';
        }
        $js = implode('', $js);
        ...
        {
            $css[] = '<link rel="stylesheet" href="' .$this->_ci->config->base_url(). assets_url('css/' . $css_file) . '">';
        }
        $css = implode('', $css);
